...
| `sys.token.max-size` | 单用户 Token 数量限制上限。 | `2` | Redis Token 存储 | 只有开关启用时生效，生产策略待确认。 | 已验证 |
...
| Token 存储 | Redis Token 存储 | `sys.token.max-size`、Redis 序列化开关、Token 限制开关 | Redis key：`token::{type}::{id}` | Redis 权限 | 无 | 已验证，开关值待确认 |

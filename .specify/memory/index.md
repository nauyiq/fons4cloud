...
| 独立认证 | `.specify/memory/capabilities/authentication-standalone/` | 已建模 |
...
| 独立认证 | `authentication-standalone` | 负责为不接入中心化认证服务与网关的单点应用提供独立的登录认证、会话校验、令牌管理（登出/踢人/查询）、会话 Redis 持久化、路由拦截鉴权与权限/角色注解接入能力。 | 已确认 | `.specify/memory/capabilities/authentication-standalone/独立认证能力文档.md` | `.specify/memory/capabilities/authentication-standalone/独立认证运行文档.md` | `.specify/memory/capabilities/authentication-standalone/独立认证配置与资源文档.md` | 已建模 |
...
| 认证鉴权与安全资源扫描 | 云基础能力框架 | `fons4cloud-auth` 提供 auth core、spring security、service API、认证服务和独立认证（Sa-Token）能力。 | 是 | 已建模：`.specify/memory/capabilities/authorization-service/`、`.specify/memory/capabilities/authentication-standalone/` |

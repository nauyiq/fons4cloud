-- 框架控制面 admin 自有库初始化 DDL 草稿。
-- 执行前提：仅供用户或 DBA 审核后手动执行；执行前需确认目标环境、库名和数据保留策略。
-- 推荐库名：sys_admin。
-- 回滚说明：先停止 admin-service，确认审计、快照和发布记录已按要求归档，再按文件末尾顺序删除表。

CREATE SCHEMA IF NOT EXISTS `sys_admin` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
USE `sys_admin`;

CREATE TABLE IF NOT EXISTS `admin_user` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `account_id` bigint NOT NULL COMMENT '认证服务账户ID；admin 不保存 source_client_id，统一通过 sys-admin 客户端边界接入',
...
CREATE TABLE IF NOT EXISTS `admin_governance_resource` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
CREATE TABLE IF NOT EXISTS `admin_governance_change` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  ...
  `validation_result` longtext DEFAULT NULL COMMENT '校验结果JSON',
...
CREATE TABLE IF NOT EXISTS `admin_governance_release` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
CREATE TABLE IF NOT EXISTS `admin_governance_snapshot` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `resource_id` bigint NOT NULL COMMENT '治理资源ID',
...
-- 普通索引设计依据：
-- 1. 账号、角色、权限表仅保留唯一约束、启停筛选和权限反查所需索引。
-- 2. 变更、发布、快照、审计表保留列表查询、状态筛选、发布追踪、回滚追踪、清理任务和审计追踪所需组合索引。
-- 3. 未为 hash、错误码、创建/更新人、授权时间等字段单独建索引；后续只有在真实查询路径和数据量证明需要时再通过 CR 追加。

-- 回滚 SQL 草稿，使用前必须先确认数据保留要求：
-- DROP TABLE IF EXISTS `admin_governance_audit`;
-- DROP TABLE IF EXISTS `admin_governance_snapshot`;
-- DROP TABLE IF EXISTS `admin_governance_release`;
-- DROP TABLE IF EXISTS `admin_governance_change`;
-- DROP TABLE IF EXISTS `admin_governance_resource`;

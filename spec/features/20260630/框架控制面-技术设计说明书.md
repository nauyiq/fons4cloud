...
`fons4cloud-admin-api` 提供控制面内部契约、DTO、错误码和后续可复用的 Dubbo API。`fons4cloud-admin-service` 提供后端管理 API、变更编排、审计、快照、发布适配、只读探测能力，并根据 CR-002 内置 Freemarker 控制台页面。
...
flowchart LR
  ...
  Browser["浏览器<br/>/admin-ui/**"] --> Page["Freemarker 页面壳<br/>轻量 JS"]
  Page --> AdminApi
...
| `fons4cloud-admin-service` | 依赖 admin-api、common-web、common-db、common-cache、common-limiter、auth-api 或 auth 内部治理契约、Spring Cloud Alibaba Nacos、Freemarker | 提供 REST API、治理编排和内置控制台页面 |
...
CR-002 采用 Freemarker 页面壳加轻量 JS 调用 API，不引入独立前端工程。页面源码、模板和静态资源均放在 `fons4cloud-admin-service` 内部：

```text
fons4cloud-admin-service
├── src/main/java/com/fons/cloud/admin/interfaces/page
├── src/main/resources/template/admin
└── src/main/resources/static/admin
```

页面路由统一使用 `/admin-ui` 前缀。页面壳不直接渲染敏感治理数据，真实数据通过浏览器调用 `/admin/**` API 获取。

| 页面 | 路径 | 数据来源 | 说明 |
| --- | --- | --- | --- |
| 登录页 | `/admin-ui/login` | `POST /admin/auth/login` | 免 Token 页面壳，登录成功后保存 `accessToken` |
| 控制台首页 | `/admin-ui` | 多个 `/admin/**` API | 展示当前治理入口和基础状态 |
| 服务治理 | `/admin-ui/services` | `/admin/services` | 服务列表和实例列表 |
| 网关治理 | `/admin-ui/gateway` | `/admin/gateway/routes/drafts`、`/admin/changes` | 路由草稿创建和变更入口 |
| 流量治理 | `/admin-ui/traffic` | `/admin/traffic/ip-lists/drafts`、`/admin/changes` | IP 黑白名单草稿入口 |
| 授权治理 | `/admin-ui/access` | `/admin/access/resources/drafts`、`/admin/changes` | 授权资源草稿入口 |
| OAuth Client 治理 | `/admin-ui/clients` | `/admin/clients/drafts`、`/admin/changes` | 客户端草稿入口 |
| 变更中心 | `/admin-ui/changes` | `/admin/changes` | 草稿、校验、发布、回滚 |
| 审计中心 | `/admin-ui/audits` | `/admin/audits` | 审计列表和详情 |
| 只读探测 | `/admin-ui/observability` | `/admin/observability/actuator/probe` | Actuator 只读探测 |

第一版 Token 处理采用 `sessionStorage` 保存 `accessToken` 和 `refreshToken`，统一 API helper 注入 `Authorization: Bearer <token>`。该方式不改变后端 Token 契约；后续如需要服务端会话或 HttpOnly Cookie，可作为独立安全增强 CR 处理。
...
- `/admin-ui/**` Freemarker 页面壳不得直接渲染服务列表、配置正文、审计详情、Token、clientSecret 等敏感数据；页面加载后通过受保护 `/admin/**` API 获取数据。
- 第一版前端 Token 保存在 `sessionStorage`，仅用于当前浏览器会话 API 调用；页面脚本不得把 Token 写入 URL、日志、模板模型或本地持久存储。
- 页面菜单隐藏、按钮禁用和前端路由控制只用于体验优化，不作为授权依据；所有权限拒绝以 gateway 和 admin-service RBAC 为准。
...
| 内置控制台技术路线 | Freemarker 页面壳 + 轻量 JS 调 `/admin/**` API | Vue/Vite 单页应用；纯服务端渲染所有数据 | 复用项目已有模板引擎方向，不引入 Node 构建链；避免页面 Controller 承载业务查询和敏感数据模型 |
| 页面源码位置 | 放在 `fons4cloud-admin-service` 内部 | 单独 `admin-ui` 模块或外部前端仓库 | 用户明确要求前端源码放进 `admin-service`；最终由 `admin-service.jar` 一体发布 |
...
| 页面测试 | Freemarker 页面渲染、静态资源访问、登录 Token 保存、API Bearer 头注入、401/403 跳转和提示 |
...
| AC-010 内置控制台 | `/admin-ui/**` Freemarker 页面、轻量 JS API helper、登录和治理页面 |
...
| 内置页面泄露敏感信息 | 未登录或低权限用户可能看到配置正文、Token 或密钥摘要 | Freemarker 只渲染页面壳，真实数据通过受保护 API 获取；模板和 JS 测试检查敏感字段不输出 |

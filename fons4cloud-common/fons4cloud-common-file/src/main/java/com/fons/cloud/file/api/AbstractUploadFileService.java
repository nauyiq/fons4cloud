...
import com.fons.cloud.common.base.lang.exception.UploadFileException;
import com.fons.cloud.file.common.FileConstants;
import com.fons.cloud.file.common.MultipartFileAdaptor;
import com.fons.cloud.file.common.annotation.UploadMode;
import com.fons.cloud.file.common.result.UploadResponse;
import com.fons.cloud.file.common.result.UploadResult;
import com.fons.cloud.file.config.UploadFileProperties;
import com.fons.cloud.file.core.UploadContext;
import com.fons.cloud.util.ImageUtil;
import com.fons.cloud.util.config.ConfigurationContext;
import com.fons.cloud.file.common.utils.FileUtil;
import com.fons.cloud.file.common.utils.FileValidateContext;
import com.fons.cloud.util.thread.NamedThreadFactory;
...
import static com.fons.cloud.common.base.lang.StringConstants.DO_PNG;
import static com.fons.cloud.util.ImageUtil.DOWNLOAD_TMP_FOLDER;
...
public abstract class AbstractUploadFileService implements UploadFileService {
    ...
    public UploadResponse uploadAvatar(MultipartFile file) throws UploadFileException {
        return uploadImgFile(FileConstants.AVATAR_FOLDER, file);
    }
...
//        String outputPath = generatorFolder + StrUtil.SLASH + fileName;
    ...
    public UploadResponse generateFile(List<String> files, String folder) throws UploadFileException {
        ...
        try {
            ...
            return writeFile(fileName, folder, UploadContext.DEFAULT_STATE , multipartFile);
        } catch (Exception e) {
            throw new UploadFileException("Failed execute generator file: " + outputPath);
        } finally {
            if (file.exists()) {
    ...
    private UploadContext.UploadState getUploadState() {
        ...
        if (state == null) {
            return UploadContext.DEFAULT_STATE;
        }
        return state;
    }
